...
class ApiAgentController extends Controller
{
    ...
    public function getHomepageInfo(Request $request)
    {
            ...
            $passedrounds = roundlist::whereNotNull('rightNumber')->orderBy('id', 'desc')->take(10)->get();
            $totalResults = roundlist::whereNotNull('rightNumber')->orderBy('id', 'desc')->take(3)->get();
...
//            }
    ...
    public function confirmBet(Request $request)
    {
        try{
            ...
            $duplicate = false;
            $nround = round::get()->first();
            if ($nround->roundname == $request->round) {
                ...
                $betlist->save();
                $lastround = roundlist::where('round', $request->round)->orderBy('id', 'desc')->get()->first();
                $lastround->totalbet = $lastround->totalbet + $request->totalbet;
                $lastround->save();
                $us = Admin::where('name', $user->name)->get()->first();
                $us->amount = $us->amount - $request->totalbet;
                $us->save();
